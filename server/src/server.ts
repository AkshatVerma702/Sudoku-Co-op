...
app.use('/routes/score', scoreRoute)
...
io.on('connection', (socket: any) => {
    ...
    socket.on('genBoards', (board: number[][]) => {
    ...
    socket.on('updateButton', (data: string) => {
        io.emit("updateButtonState", data);
    });

    socket.on("readyPlayer", (socketId: any) => {
    ...
    socket.on('submitBoards', async(obj: any) => {
